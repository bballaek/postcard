...
export type PostcardStyle = "modern" | "retro" | "vintage";

export function themeKey(raw: string | null | undefined): PostcardStyle {
...
type FrameBackgroundOpts = {
  sheet: HTMLElement | null;
  stamp: SVGSVGElement | null;
  stampFill: SVGRectElement | null;
  stampMask: SVGMaskElement | null;
  stampMaskFill: SVGRectElement | null;
  stampHoleCircles: SVGGElement | null;
  insetVar?: string;
};

export function createFrameBackground(opts: FrameBackgroundOpts) {
  ...
  function getInset() {
    return (
      parseFloat(
        getComputedStyle(document.documentElement).getPropertyValue(insetVar),
      ) || 30
    );
  }
  ...
  function fitStampSheet() {
    ...
    if (stampMask) {
      ...
      stampMask.setAttribute("width", String(w + maskPad * 2));
      stampMask.setAttribute("height", String(h + maskPad * 2));
    }
    if (stampMaskFill) {
      stampMaskFill.setAttribute("x", "0");
      stampMaskFill.setAttribute("y", "0");
      stampMaskFill.setAttribute("width", String(w));
      stampMaskFill.setAttribute("height", String(h));
    }
    stampFill.setAttribute("x", "0");
    stampFill.setAttribute("y", "0");
    stampFill.setAttribute("width", String(w));
    stampFill.setAttribute("height", String(h));
    ...
    function addStampHole(cx: number, cy: number) {
